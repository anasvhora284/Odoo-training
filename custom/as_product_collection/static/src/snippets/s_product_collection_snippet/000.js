...
publicWidget.registry.ProductCollectionSnippet = publicWidget.Widget.extend({
  ...
  _createProductCard(product) {
    const isInWishlist = sessionStorage
      .getItem("wishlist")
      ?.includes(product.product_tmpl_id);
      ...
      <div class="card border-1 rounded-2 h-100 w-100 oe_product_cart js_product o_carousel_product_card" data-product-id="${
        product.id
          ...
          <a href="${productLink}" class="text-decoration-none">
            <img src="${
              product.image_url
            }" class="card-img-top object-fit-cover px-0 p-lg-0" alt="${
      product.name
    }"/>
          </a>
          <div class="card-body">
  ...
  openConfigDialog(event) {
    ...
    const productTemplateId = parseInt(
      event.currentTarget.dataset.productTemplateId
    );
          ...
          const optionalProductsData = optionalProducts
            .map((product) => {
              ...
              if (!variantId || !templateId) {
                console.warn(
                  "Missing required IDs for optional product:",
                  product
                );
                return null;
              }
